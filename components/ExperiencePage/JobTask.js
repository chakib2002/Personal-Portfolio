    ...
    <div className="mx-5 lg:mx-10 text-white">
      <div>
        <h1 className="text-lg font-semibold">
          {title}{" "}
          <span className="text-green-400 font-normal text-base lg:hover:underline lg:hover:underline-offset-4 cursor-pointer">
            @{company}
          </span>
        </h1>
        ...
        <div className="space-y-5 ">
          {jobDetails?.map((element, index) => (
            <div key={index} className="flex space-x-2">
              ...
              <p className="text-sm text-left justify-self-start  text-slate-400  inline">
                {element}
              </p>
            </div>
          ))}
